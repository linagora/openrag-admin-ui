<script lang="ts">
	...
	let selectedExtractContent: RagondinExtract | null = null; // The content of the selected document
	...
	onMount(async () => {
		...
		if (!$currentPartition?.partition) {
			...
			goto(`/partition/${$currentPartition.partition}`);
			return;
		}

		file = await api.fetchFile($currentPartition?.partition, $currentFile);
	});
</script>
...
{#if file && file.documents}
	<div class="p4 flex h-full max-h-full divide-x divide-slate-200">
		<div class="flex w-128 flex-col bg-slate-50">
			<span class="top-0 w-full border-b border-slate-200 bg-white px-4 py-2 font-bold">
				File metadata
			</span>
			...
			<div class="flex max-h-full flex-col items-center divide-y divide-slate-200 overflow-y-auto">
				{#each file?.documents as document}
					...
					>
						...
						<ChevronDown
							className="-rotate-90 size-4 stroke-2 {document.link.split('/').pop() ===
							selectedExtract
		...
		<div class="flex w-7/12 max-w-7/12 flex-col bg-slate-50">
			<span class="top-0 border-b border-slate-200 bg-white px-4 py-2 text-center font-bold">
				Preview
			</span>
