<script lang="ts">
    ...
    function handleInput(event: Event) {
        ...
        if (input.value !== "") {
            ...
            authTokenInput.placeholder = $_('login.enter_token');
        }
    }
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            authTokenInput.placeholder = $_('login.incorrect_token');
            animateError();
        }
    }
...
<div class="flex flex-col items-center justify-end relative h-[50vh] w-screen">
    ...
    <span class="font-bold text-2xl mb-6">{$_('login.access_protected')}</span>
    <div class="relative flex items-center">
        <input
            bind:this={authTokenInput}
            bind:focused
            type="password"
            placeholder={$_('login.enter_token')}
