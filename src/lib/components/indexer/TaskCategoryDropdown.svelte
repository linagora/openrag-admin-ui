...
<div class={opened ? "h-full overflow-y-auto" : ""}>
    <button
        ...
        class="sticky top-0 z-10 flex w-full cursor-pointer items-center justify-center space-x-1 border-t border-slate-200 bg-slate-50 py-2 hover:bg-slate-100
		{opened ? ' border-b shadow' : ''}"
    >
        <span class="text-xs font-semibold text-slate-500">
            {category} ({indexerData.tasks.filter((task) => task.state === category).length})
        </span>
        <ChevronDown
    ...
    {#if opened}
        <div class="divide-y divide-slate-200 h-full">
            ...
            {#if category === "COMPLETED"}
                {#each indexerData.tasks.filter((task) => task.state === category) as task}
                    <a
                        href={`/indexer/partition/${task.details.partition}/file/${task.details.file_id}`}
