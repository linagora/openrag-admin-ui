<script lang="ts">
	...
	// Import types
	import type { RagondinTaskStatus } from '$lib/types';
...
<div class={opened ? 'h-full overflow-y-auto' : ''}>
	...
	>
		<span class="text-xs font-semibold text-slate-500">
			{category} ({$tasks.filter((task) => task.state === category).length})
		</span>
		<ChevronDown
	...
	{#if opened}
		<div class="divide-y divide-slate-200">
			...
			{#if category === 'COMPLETED'}
				{#each $tasks.filter((task) => task.state === category) as task}
					<a
